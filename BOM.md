...
| Device | Qty | Notes |
|---|---|---|
| Gas Sensor | 1 | `sensor_600` (GS-601) — also wired on a second cable into Area 800's `sensor_800`, same physical device |
...
| Device | Qty | Notes |
|---|---|---|
| Gas Sensor | 0 | `sensor_800` reads GS-601 (Area 600) on a second cable — no separate Gas Sensor needed here |
...
## Totals
- **Structures (fixed quantities): 74** — 4 Large Powered Vent, 5 Turbo Volume Pump, 1 Volume Pump, 10 Pipe Analyzer, 1 Gas Sensor (shared by Area 600 and Area 800 on two cables to the same device), 6 Digital Valve, 1 Liquid Digital Valve, 1 Indicator Light, 10 Filtration, 2 Ice Crusher, 4 Condensation Valve, 1 Purge Valve, 1 Large Tank (Insulated), 2 Large Liquid Tank (Insulated), 5 Tank, 10 IC Housing, 10 Integrated Circuit (IC10)
